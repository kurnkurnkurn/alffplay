...
struct AudioState {
    ...
    nanoseconds getClockNoLock() const;
    nanoseconds getClock()
    {
        std::lock_guard lock{mSrcMutex};
...
nanoseconds AudioState::getClockNoLock() const
{
    ...
    if(alcGetInteger64vSOFT)
    {
        ...
        const auto latency = nanoseconds{devtimes[1]};
        const auto device_time = nanoseconds{devtimes[0]};
...
int AudioState::getSync()
{
    ...
    if(mMovie.mAVSyncType == SyncMaster::Audio)
        return 0; // 如果是音频同步，直接返回0，不需要进一步处理
...
// 函数功能：解码音频帧并进行格式转换，返回有效采样数
// 返回值：转换后的有效音频采样数（负值为错误）
int AudioState::decodeFrame()
{
    // 循环解码直到获取有效音频样本（处理空帧情况）
    do {
        // 从队列获取已解码的音频帧（线程安全操作）
        while(int ret{mQueue.receiveFrame(mCodecCtx.get(), mDecodedFrame.get())})
        {
            if(ret == AVErrorEOF)  // 遇到流结束标志[6,10](@ref)
                return 0;
            fmt::println(stderr, "Failed to receive frame: {}", ret); // 打印FFmpeg错误码[8](@ref)
        }
    } while(mDecodedFrame->nb_samples <= 0);       // 确保获取有效音频样本[6](@ref)
    ...
    if(mDecodedFrame->best_effort_timestamp != AVNoPtsValue)
        mCurrentPts = duration_cast<nanoseconds>(seconds_d64{av_q2d(mStream->time_base) *
            static_cast<double>(mDecodedFrame->best_effort_timestamp)});  // 时间基转换[6,7](@ref)

    /* 内存管理 ------------------------------------------------------*/
    // 动态调整采样缓冲区大小（应对变长音频帧）
    if(mDecodedFrame->nb_samples > mSamplesMax)
    {
        av_freep(static_cast<void*>(mSamples.data()));  // 释放旧缓冲区[6](@ref)
        // 分配新缓冲区：通道数×采样数×采样格式
        av_samples_alloc(mSamples.data(), nullptr,
            mCodecCtx->ch_layout.nb_channels,
            mDecodedFrame->nb_samples,
            mDstSampleFmt, 0);                          // 内存对齐参数0[6,11](@ref)
        mSamplesMax = mDecodedFrame->nb_samples;         // 更新最大采样容量
        mSamplesSpan = {mSamples[0],
            static_cast<size_t>(mSamplesMax)*mFrameSize};// 创建数据视图[3](@ref)
    }

    /* 音频重采样 ----------------------------------------------------*/
    // 执行格式转换（如PCM16→PCM32/单声道→立体声）
    const int data_size{swr_convert(mSwresCtx.get(),
        mSamples.data(),                 // 目标缓冲区
        mDecodedFrame->nb_samples,       // 目标最大采样数
        mDecodedFrame->extended_data,    // 源数据指针数组[6](@ref)
        mDecodedFrame->nb_samples)};     // 源采样数

    av_frame_unref(mDecodedFrame.get());  // 释放帧引用（避免内存泄漏）[6](@ref)
    return data_size;  // 返回实际转换采样数（可能小于请求值）
}
 ...
 */
template<typename T>
void sample_dup(std::span<uint8_t> out, std::span<const uint8_t> in, size_t count, size_t frame_size)
{
...
// 函数功能：将解码后的音频数据填充到环形缓冲区，处理跳帧并更新播放时间戳
// 参数 sample_skip：需要跳过的初始采样数（用于音视频同步或调试）
// 返回值：true-数据填充成功；false-解码失败或数据不足
bool AudioState::readAudio(int sample_skip)
{
    // 原子获取当前写指针位置（保证多线程可见性）
    size_t woffset{mWritePos.load(std::memory_order_acquire)};
    // 宽松模式读取读指针（无需严格同步，读线程可能滞后）
    const size_t roffset{mReadPos.load(std::memory_order_relaxed)};

    // 主循环：处理当前音频帧数据（mSamplesLen > 0表示有待处理数据）
    while(mSamplesLen > 0)
    {
        // 计算环形缓冲区当前可写入的采样数（考虑环形缓冲区回绕）
        // 分三种情况处理写指针与读指针的相对位置：
        // 1. 写指针在读指针前方（roffset > woffset）：可写入到读指针前一个位置
        // 2. 读指针在起始位置（roffset == 0）：可写入到缓冲区末尾
        // 3. 其他情况：可写入到缓冲区末尾
        const size_t nsamples{
            ((roffset > woffset) ? roffset - woffset - 1
             : (roffset == 0) ? (mBufferData.size() - woffset - 1)
             : (mBufferData.size() - woffset)) / mFrameSize // 转换为采样数
        };
        if(!nsamples) break; // 无可用空间则退出循环

        // 处理静音填充模式（mSamplesPos < 0 表示需要生成静音）
        if(mSamplesPos < 0)
        {
            // 计算需要填充的静音采样数（不超过当前可写空间和剩余静音需求）
            const size_t rem{std::min<size_t>(nsamples, static_cast<ALuint>(-mSamplesPos))};

            // 生成静音数据（使用span安全访问缓冲区）
            sample_dup(
                std::span{mBufferData}.subspan(woffset), // 目标缓冲区区间
                mSamplesSpan,                        // 源数据（可能为空或静音模板）
                rem,                                    // 采样数
                mFrameSize                              // 单采样字节数
            );
            ...
            if(woffset == mBufferData.size()) woffset = 0; // 环形缓冲区回绕
            mWritePos.store(woffset, std::memory_order_release); // 释放写指针更新

            // 更新时间戳：采样数转换为时间增量
            mCurrentPts += nanoseconds{seconds{rem}} / mCodecCtx->sample_rate;
            mSamplesPos += static_cast<int>(rem); // 减少待填充静音数
            continue; // 继续处理下一个数据块
        }

        // 正常数据填充模式-----------------------------------------------
        // 计算可拷贝的采样数（不超过可用空间和当前帧剩余数据）
        const size_t rem{std::min<size_t>(nsamples, static_cast<ALuint>(mSamplesLen - mSamplesPos))};
        const size_t boffset{static_cast<ALuint>(mSamplesPos) * size_t{mFrameSize}}; // 源数据偏移
        const size_t nbytes{rem * mFrameSize}; // 待拷贝字节数

        // 安全拷贝数据（使用span迭代器替代原始指针）
        std::copy_n(
            mSamplesSpan.cbegin() + static_cast<ptrdiff_t>(boffset), // 源起始位置
            nbytes,                                     // 拷贝字节数
            mBufferData.begin() + static_cast<ptrdiff_t>(woffset)    // 目标起始位置
        );
        ...
        // 更新时间戳
        mCurrentPts += nanoseconds{seconds{rem}} / mCodecCtx->sample_rate;
        mSamplesPos += static_cast<int>(rem); // 移动当前帧读取位置

        // 解码新帧处理--------------------------------------------------
        // 当当前帧数据耗尽时，解码新帧
        while(mSamplesPos >= mSamplesLen)
        {
            mSamplesLen = decodeFrame(); // 解码新音频帧（返回新帧的采样数）
            mSamplesPos = std::min(mSamplesLen, sample_skip); // 应用跳帧
            if(mSamplesLen <= 0) return false; // 解码失败则返回错误

            sample_skip -= mSamplesPos; // 减少剩余需跳过的采样数

            // 调整设备启动时间（补偿跳过的采样）
            auto skip = nanoseconds{seconds{mSamplesPos}} / mCodecCtx->sample_rate;
            mDeviceStartTime -= skip;  // 补偿设备时钟
            mCurrentPts += skip;       // 推进时间戳
        }
    }
...
{
    std::unique_lock srclock{mSrcMutex, std::defer_lock};
    ...
    struct EventControlManager {
        ...
        ~EventControlManager()
        {
            if(alEventControlSOFT)
            {
                alEventControlSOFT(static_cast<ALsizei>(evt_types.size()), evt_types.data(), AL_FALSE);
    ...
    auto sender [[maybe_unused]] = std::async(std::launch::async, packet_sender); // 立刻启动
...
int VideoState::handler()
{
    ...
    {
        std::lock_guard displock{mDispPtsMutex};
...
auto PrettyTime(const seconds t) -> std::string
{
...
int main(std::span<std::string_view> args)
{
    ...
    printALCInfo(alcGetContextsDevice(alcGetCurrentContext()));
    ...
    while(true)
    {
        ...
        if(cur_time != last_time)
        {
            ...
            // fmt::print("    \r {} / {}", cur_time.count(), end_time.count());
            fflush(stdout);
            last_time = cur_time;
        }
